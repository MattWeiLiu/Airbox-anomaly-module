...
msg = {}
msg["source"] = str("device_malfunction_weekly by IIS-NRL").encode("utf-8")
msg["feeds"] = feeds
...
with open(DIR_DATAANALYSIS+'device_malfunction_weekly.json','w') as fout:
	json.dump(msg,fout)
...
utc_datetime = datetime.datetime.utcnow()
msg["version"] = utc_datetime.strftime("%Y-%m-%dT%H:%M:%SZ")
with open(DIR_DATAANALYSIS+'device_indoor_7.json','w') as fout:
	json.dump(msg,fout)
...
utc_datetime = datetime.datetime.utcnow()
msg["version"] = utc_datetime.strftime("%Y-%m-%dT%H:%M:%SZ")
with open(DIR_DATAANALYSIS+'device_emission_7.json','w') as fout:
	json.dump(msg,fout)
